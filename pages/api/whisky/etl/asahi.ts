...
const FEEDS = [
  ...
  { key: "asahi_beer", url: process.env.ASAHI_RSS_BEER },
...
const ASAHI_RE = new RegExp(
  "(アサヒ|Asahi|アサヒビール|Asahi Beer|アサヒグループ|Asahi Group|スーパードライ|Super Dry|ドライゼロ|Dry Zero|ザ・マスターズ|The Master's|アサヒスタイルフリー|Style Free|アサヒオフ|Asahi Off|アサヒドライ|Asahi Dry|アサヒビール|Asahi Breweries|アサヒグループホールディングス|Asahi Group Holdings|アサヒ飲料|Asahi Soft Drinks|三ツ矢|Mitsuya|ウィルキンソン|Wilkinson|カルピス|Calpis|ポッカサッポロ|Pocari Sweat|オランジーナ|Orangina|アサヒフード&ヘルスケア|Asahi Food & Healthcare)",
  "i"
);
  ...
  try {
    ...
    if (FEEDS.length === 0) {
      return res.status(500).json({ 
        error: "No RSS feeds configured. Please set environment variables: ASAHI_RSS_GROUP, ASAHI_RSS_BEER" 
      });
    }
    ...
    for (const f of FEEDS) {
      ...
      for (const it of items) {
        ...
        const text = `${title}\n${desc}\n${html}`;
        if (!ASAHI_RE.test(text)) continue;
    ...
    if (!payload.length) {
      return res.status(200).json({ inserted: 0, note: "no Asahi items matched" });
    }
    ...
    console.error("Asahi ETL error:", e);
    res.status(500).json({ error: e?.message || "asahi etl failed" });
  }
}
