...
const FEEDS = [
  { key: "prtimes_kirin", url: process.env.PRTIMES_RSS_KIRIN },
  { key: "google_kirin", url: process.env.GOOGLE_RSS_KIRIN },
].filter((x): x is { key: string; url: string } => !!x.url);

const KIRIN_RE = new RegExp(
  "(キリン|Kirin|富士御殿場|Fuji Gotemba|キリンディスティラリー|Kirin Distillery|ウイスキー|Whisky|Whiskey)",
  "i"
);
  ...
  try {
    ...
    if (FEEDS.length === 0) {
      return res.status(500).json({ 
        error: "No RSS feeds configured. Please set environment variables: PRTIMES_RSS_KIRIN, GOOGLE_RSS_KIRIN" 
      });
    }
    ...
    for (const f of FEEDS) {
      ...
      for (const it of items) {
        ...
        const text = `${title}\n${desc}\n${html}`;
        if (!KIRIN_RE.test(text)) continue;
    ...
    if (!payload.length) {
      return res.status(200).json({ inserted: 0, note: "no Kirin items matched" });
    }
    ...
    console.error("Kirin ETL error:", e);
    res.status(500).json({ error: e?.message || "kirin etl failed" });
  }
}
