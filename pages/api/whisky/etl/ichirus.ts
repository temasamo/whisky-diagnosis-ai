...
const FEEDS = [
  { key: "prtimes_ichirus", url: process.env.PRTIMES_RSS_ICHIRUS },
  { key: "google_ichirus", url: process.env.GOOGLE_RSS_ICHIRUS },
].filter((x): x is { key: string; url: string } => !!x.url);

const ICHIRUS_RE = new RegExp(
  "(イチローズモルト|Ichiro's Malt|ベンチャーウイスキー|Venture Whisky|秩父|Chichibu|ウイスキー|Whisky|Whiskey)",
  "i"
);
...
async function fetchFeed(url: string) {
  const xml = await fetch(url, {
    headers: {
      ...
      referer: "https://www.venturewhisky.com/",
    },
    redirect: "follow",
    cache: "no-store",
  }).then(r => r.text());
  ...
  try {
    ...
    if (FEEDS.length === 0) {
      return res.status(500).json({ 
        error: "No RSS feeds configured. Please set environment variables: PRTIMES_RSS_ICHIRUS, GOOGLE_RSS_ICHIRUS" 
      });
    }
    ...
    for (const f of FEEDS) {
      ...
      for (const it of items) {
        ...
        const text = `${title}\n${desc}\n${html}`;
        if (!ICHIRUS_RE.test(text)) continue;
    ...
    if (!payload.length) {
      return res.status(200).json({ inserted: 0, note: "no Ichiro's Malt items matched" });
    }
    ...
    console.error("Ichiro's Malt ETL error:", e);
    res.status(500).json({ error: e?.message || "ichirus etl failed" });
  }
}
