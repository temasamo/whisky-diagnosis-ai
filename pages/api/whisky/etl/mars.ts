...
const FEEDS = [
  { key: "prtimes_mars", url: process.env.PRTIMES_RSS_MARS },
  { key: "google_mars", url: process.env.GOOGLE_RSS_MARS },
].filter((x): x is { key: string; url: string } => !!x.url);

const MARS_RE = new RegExp(
  "(マルス|Mars|本坊酒造|Hombo|信州|Shinshu|津貫|Tsunuki|マルスウイスキー|Mars Whisky|ウイスキー|Whisky|Whiskey)",
  "i"
);
  ...
  try {
    ...
    if (FEEDS.length === 0) {
      return res.status(500).json({ 
        error: "No RSS feeds configured. Please set environment variables: PRTIMES_RSS_MARS, GOOGLE_RSS_MARS" 
      });
    }
    ...
    if (!payload.length) {
      return res.status(200).json({ inserted: 0, note: "no Mars items matched" });
    }
    ...
    console.error("Mars ETL error:", e);
    res.status(500).json({ error: e?.message || "mars etl failed" });
  }
}
