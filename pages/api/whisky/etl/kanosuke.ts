...
const FEEDS = [
  { key: "prtimes_kanosuke", url: process.env.PRTIMES_RSS_KANOSUKE },
  { key: "google_kanosuke", url: process.env.GOOGLE_RSS_KANOSUKE },
].filter((x): x is { key: string; url: string } => !!x.url);

const KANOSUKE_RE = new RegExp(
  "(嘉之助|Kanosuke|小正醸造|Komasa|ウイスキー|Whisky|Whiskey)",
  "i"
);
...
async function fetchFeed(url: string) {
  const xml = await fetch(url, {
    headers: {
      ...
      referer: "https://www.kanosuke-distillery.com/",
    },
    redirect: "follow",
    cache: "no-store",
  }).then(r => r.text());
  ...
  try {
    ...
    if (FEEDS.length === 0) {
      return res.status(500).json({ 
        error: "No RSS feeds configured. Please set environment variables: PRTIMES_RSS_KANOSUKE, GOOGLE_RSS_KANOSUKE" 
      });
    }
    ...
    for (const f of FEEDS) {
      ...
      for (const it of items) {
        ...
        const text = `${title}\n${desc}\n${html}`;
        if (!KANOSUKE_RE.test(text)) continue;
    ...
    if (!payload.length) {
      return res.status(200).json({ inserted: 0, note: "no Kanosuke items matched" });
    }
    ...
    console.error("Kanosuke ETL error:", e);
    res.status(500).json({ error: e?.message || "kanosuke etl failed" });
  }
}
