...
const FEEDS = [
  { key: "prtimes_nagahama", url: process.env.PRTIMES_RSS_NAGAHAMA },
  { key: "google_nagahama", url: process.env.GOOGLE_RSS_NAGAHAMA },
].filter((x): x is { key: string; url: string } => !!x.url);

const NAGAHAMA_RE = new RegExp(
  "(長濱|Nagahama|AMAHAGAN|アマハガン|長濱蒸溜所|Nagahama Distillery|ウイスキー|Whisky|Whiskey)",
  "i"
);
...
async function fetchFeed(url: string) {
  const xml = await fetch(url, {
    headers: {
      ...
      referer: "https://www.nagahama-roman.com/",
    },
    redirect: "follow",
    cache: "no-store",
  }).then(r => r.text());
  ...
  try {
    ...
    if (FEEDS.length === 0) {
      return res.status(500).json({ 
        error: "No RSS feeds configured. Please set environment variables: PRTIMES_RSS_NAGAHAMA, GOOGLE_RSS_NAGAHAMA" 
      });
    }
    ...
    for (const f of FEEDS) {
      ...
      for (const it of items) {
        ...
        const text = `${title}\n${desc}\n${html}`;
        if (!NAGAHAMA_RE.test(text)) continue;
    ...
    if (!payload.length) {
      return res.status(200).json({ inserted: 0, note: "no Nagahama items matched" });
    }
    ...
    console.error("Nagahama ETL error:", e);
    res.status(500).json({ error: e?.message || "nagahama etl failed" });
  }
}
