...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    for (const release of allReleases || []) {
      const expr = release.expression_id ? expressionsMap.get(release.expression_id) : null;
      ...
      if (hasNonWhiskyKeyword) {
        ...
        // 優先順位2: ウイスキー関連キーワードが含まれているかチェック
      ...
      // データベースに保存（categoryカラムが存在する場合）
      // 注意: テーブルにcategoryカラムが存在しない場合はエラーになります
      ...
      if (updateError) {
        // categoryカラムが存在しない場合は、エラーを記録して続行
        if (updateError.message.includes("column") && updateError.message.includes("does not exist")) {
          return res.status(500).json({
            error: "`releases`テーブルに`category`カラムが存在しません",
            hint: "Supabaseで`releases`テーブルに`category`カラム（text型）を追加してください",
            details: updateError.message,
          });
        }
        errors++;
        results.push({ id: release.id, error: updateError.message });
      } else {
        updated++;
        results.push({ 
          id: release.id, 
    ...
    res.status(200).json({
      ...
      updated,
      errors,
      message: `${updated}件のリリース情報のカテゴリを更新しました`,
      results: results.slice(0, 10), // 最初の10件だけ返す
    });
  } catch (error: any) {
